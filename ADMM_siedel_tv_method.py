...
def update_t_r(x,y,z,t,r,opts,tv_scale_lam):
    ...
    def gradient_tv(t):
        ...
        size_2 = np.size(t,0)
        dimension = np.size(t,1)
        size = int(sqrt(size_2))
        t = np.array(t)
        t_ = t.reshape([size,size,dimension])
        gradient_1 = np.zeros([size,size,dimension])
        gradient_2 = np.zeros([size,size,dimension])
        gradient_3 = np.zeros([size,size,dimension])
        gradient_4 = np.zeros([size,size,dimension])
        gradient_1[0:size-1,:] = np.sign(t_[0:size-1,:]-t_[1:size,:])
        gradient_2[1:size,:] = np.sign(t_[1:size,:]-t_[0:size-1,:])
        gradient_3[:,0:size-1] = np.sign(t_[:,0:size-1]-t_[:,1:size])
        gradient_4[:,1:size] = np.sign(t_[:,1:size]-t_[:,0:size-1])
        gradient = gradient_1+gradient_2+gradient_3+gradient_4

        return gradient.reshape([size_2,dimension])
...
def Alter_ADMM_siedel_tv(y,z,n_centers,opts,lam=1,verbose = False):
    '''
    ...
    while iter < max_iter:
        ...
        # Update t,r
        t,r = update_t_r(x,y,z,t,r,opts1,lam)
