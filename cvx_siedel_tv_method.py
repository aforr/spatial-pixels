...
def cvx_solve_tv(x,y,z,lam):
    ...
    index = np.linspace(0,num_grids-1,num_grids)
    index = index.astype(int)
    index = index.reshape([size,size])

    num   = size*(size-1)
    index1 = index[:,0:size-1].reshape(num)
    index2 = index[:,1:size].reshape(num)
    index3 = index[0:size-1,:].reshape(num)
    index4 = index[1:size,:].reshape(num)
...
def siedel_x_tv(x, y, z, t, r, opts):
    '''
...
def Alter_cvx_siedel_tv(y,z,n_centers,opts,lam=1,verbose = False):
    '''
    ...
    while iter < max_iter:
        ...
        t,r = cvx_solve_tv(x,y,z,lam)

        x = siedel_x_tv(x,y,z,t,r,opts2)

        if verbose == True:
            verbose_var[iter] = get_val_tv(x,y,z,t,r)
